...
- `POST /api/v1/franchises` — create franchise `{ name }` → `{ id }`
- `PATCH /api/v1/franchises/{franchiseId}/name` — update franchise name
- `POST /api/v1/franchises/{franchiseId}/branches` — add branch → `{ id }`
- `PATCH /api/v1/franchises/{franchiseId}/branches/{branchId}/name` — update branch name
- `POST /api/v1/franchises/{franchiseId}/branches/{branchId}/products` — add product → `{ id }`
- `PATCH /api/v1/franchises/{franchiseId}/branches/{branchId}/products/{productId}/stock` — update stock
- `PATCH /api/v1/franchises/{franchiseId}/branches/{branchId}/products/{productId}/name` — update product name
- `DELETE /api/v1/franchises/{franchiseId}/branches/{branchId}/products/{productId}` — remove product
- `GET /api/v1/franchises/{franchiseId}/branches/max-stock` — product with max stock per branch
