...
class UpdateProductStockUseCaseTest {
  ...
  private final UpdateProductStockUseCase useCase = new UpdateProductStockUseCase(repo);
  ...
  void updatesProductStock() {
    ...
    StepVerifier.create(useCase.execute("f-1", "b-1", "p-1", 9))
        .assertNext(product -> {
  ...
  void failsWhenFranchiseMissing() {
    ...
    StepVerifier.create(useCase.execute("missing", "b-1", "p-1", 9))
        .expectErrorSatisfies(ex -> {
          assertInstanceOf(DomainException.class, ex);
