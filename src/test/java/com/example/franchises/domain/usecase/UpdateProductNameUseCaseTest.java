...
class UpdateProductNameUseCaseTest {
  ...
  private final UpdateProductNameUseCase useCase = new UpdateProductNameUseCase(repo);
  ...
  void updatesProductName() {
    ...
    StepVerifier.create(useCase.execute("f-1", "b-1", "p-1", "Juice"))
        .assertNext(product -> {
    ...
    assertEquals("Juice", saved.name());
  }

  @Test
  void failsWhenProductMissing() {
    Branch branch = new Branch("b-1", "Downtown", List.of());
    Franchise franchise = new Franchise("f-1", "Acme", List.of(branch));
  ...
  void failsWhenFranchiseMissing() {
    ...
    StepVerifier.create(useCase.execute("missing", "b-1", "p-1", "Juice"))
        .expectErrorSatisfies(ex -> {
          assertInstanceOf(DomainException.class, ex);
